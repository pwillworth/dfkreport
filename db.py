...
def createReport(wallet, startDate, endDate, now, txCount, costBasis):
    con = aConn()
    cur = con.cursor()
    cur.execute("INSERT INTO reports VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NULL, %s)", (wallet, startDate, endDate, now, txCount, 0, 0, 0, '', '', costBasis))
    con.commit()
    con.close()
